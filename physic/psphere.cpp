...
PSphere::PSphere(float radius, Coordinates& coord) : PObject(coord)
{
    m_radius = radius;
}
...
{
    return distance2(m_coord.getPosition(), s->getCoordinates().getPosition()) < (m_radius + s->getRadius()) * (m_radius + s->getRadius());
}
...
vector<tuple<vec3,vec3>> PSphere::collisionPointsWithSphere(PSphere* obj) 
{
    ...
    vec3 point((m_radius * m_coord.getPosition() + obj->getRadius() * obj->getCoordinates().getPosition()) / (m_radius + obj->getRadius()));
    vec3 normal(m_coord.getPosition() - obj->getCoordinates().getPosition());
